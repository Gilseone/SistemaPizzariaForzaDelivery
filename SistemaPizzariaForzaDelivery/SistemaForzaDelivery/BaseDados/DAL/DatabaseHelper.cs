...
namespace BaseDados.DAL
{
	public class DatabaseHelper : IDisposable
	{
		...
		internal DbDataReader ExecuteReader(string query, CommandType commandtype, DatabaseConnectionState connectionstate)
		{
			...
			{
				throw new Exception("Erro no sql: " + exc.StackTrace);
			}
			finally
			{
				objCommand.Parameters.Clear();
		...
		/// <summary>
		/// Retorna o 
		/// Returns the number of affected rows, not the last inserted (Somente para MySql)
		/// </summary>
		/// <returns></returns>
		internal long LastInsertedId()
		{
			return	((MySqlCommand)objCommand).LastInsertedId;
		}
